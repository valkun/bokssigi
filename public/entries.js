...
function requireMore(nextPageToken) {
    let LusingDate = $('#LusingDate')[0].value;
    let RusingDate = $('#RusingDate')[0].value;
    let account = $('#account')[0].value;
    let description = $('#description')[0].value;
    axios.get(`?pageToken=${encodeURIComponent(nextPageToken)}&LusingDate=${LusingDate}&RusingDate=${RusingDate}&account=${account}&description=${description}&resType=html`, {
        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
    }).then((response) => {        
